...
class CameraController:
    ...
    def set_cam_pos(self, forced=False):
        d = self.distance
        ...
        self.position[0] = lerp(self.position[0], self.target[0] + x, 5 * dt)
        self.position[1] = lerp(self.position[1], self.target[1] + y, 5 * dt)
        self.position[2] = lerp(self.position[2], self.target[2] + h, 5 * dt)
